...
class User extends CI_Controller
{
 ...
 $this->form_validation->set_rules('new_password1', 'New Password', 'required|trim|min_length[3]|matc
hes[new_password2]');
 $this->form_validation->set_rules('new_password2', 'Confirm New Password', 'required|trim|min_length
[3]|matches[new_password1]');
 if ($this->form_validation->run() == false) {
 $this->load->view('_templates/header', $data);
 $this->load->view('_templates/navbar');
 $this->load->view('_templates/sidebar', $data);
 ...
 $this->session->set_flashdata('message', '<div class="alert alertdanger" role="alert">Kata sandi Baru tidak boleh sama dengan yang sekarang!</
div>');
 redirect('user/changepassword');
 } else {
 // password sudah ok
